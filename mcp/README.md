...
`@otok/mcp` depends on `"@otok/node": "^0.8.0"`. For development, install the SDK from this checkout as a locally packed tarball, including when testing an unreleased version. Use Node.js 20.19 or newer for development:
...
CI (`.github/workflows/ci.yml`, `mcp` job) runs the same steps. `mcp/` deliberately ships **without `package-lock.json`** for now — a lockfile would pin the `file:` tarball path; the release workflow separately checks the registry for `@otok/node@^0.8.0`.
...
1. **Publish `@otok/node` first** — the workflow refuses to publish while `@otok/node@^0.8.0` is not installable from npm.
