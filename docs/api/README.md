...
- **Plan:** requires a plan with API access (Growth or higher); deals/pipelines, payments (incl. contact documents), payment requests, orders, campaigns, and bookings/meeting-types additionally require the matching plan feature — see [feature-gated resource groups](getting-started.md#feature-gated-resource-groups)
...
| [Webhooks](webhooks.md) | Email + order + payment-request event webhooks: registration, signatures, retries |
